...
import random
from utils.helpers import get_stopwords_data, get_all_stopwords
...
async def verify_stopword_rephrasing_ai(original_sentence, rephrased_sentence, stopword, user_id=None):
    ...
    # Получаем полный список всех стоп-слов для проверки
    all_stopwords_info = get_all_stopwords()
    all_stopwords = all_stopwords_info["words_list"]
    ...
    # Добавляем полный список стоп-слов для проверки
    # Выбираем до 100 стоп-слов для передачи в API (чтобы не переполнить контекст)
    stopwords_to_check = all_stopwords
    if len(stopwords_to_check) > 100:
        # Если стоп-слов больше 100, сначала убедимся, что целевое стоп-слово в списке
        # Затем добавим случайную выборку из остальных
        if stopword_word and stopword_word.lower() not in stopwords_to_check[:99]:
            stopwords_to_check = [stopword_word.lower()] + random.sample([w for w in all_stopwords if w.lower() != stopword_word.lower()], 99)
        else:
            stopwords_to_check = random.sample(all_stopwords, 100)
    
    context += f"\nПолный список стоп-слов для проверки: {', '.join(stopwords_to_check)}\n"
...
ПРИМЕР 8:
Оригинал: "Ну да, эти цифры совпадают с нашими прогнозами."
Стоп-слово: "ну да"
Перефраз: "Эти цифры только совпадают"
❌ НЕПРАВИЛЬНО - "только" является другим стоп-словом

ПРИМЕР 9:
Оригинал: "Мы работаем только с проверенными поставщиками."
Стоп-слово: "только"
Перефраз: "Мы работаем с проверенными поставщиками."
✅ ПРАВИЛЬНО - Пользователь удалил стоп-слово "только" и сохранил основной смысл.
"""
...
2. СЧИТАЙТЕ НЕПРАВИЛЬНЫМ ОТВЕТОМ:
   ...
   - Использование ЛЮБОГО другого стоп-слова из списка стоп-слов (даже если оно отличается от оригинального)
...
4. ВНИМАТЕЛЬНО ПРОВЕРЬТЕ ЕСТЬ ЛИ В ПЕРЕФРАЗИРОВКЕ ДРУГИЕ СТОП-СЛОВА:
   Проверьте КАЖДОЕ слово в перефразированном предложении на наличие в списке стоп-слов. Если в перефразированном предложении используется ЛЮБОЕ слово из списка стоп-слов, ответ неправильный.
