...
async def generate_ai_stopword_sentence(stopword_data):
    ...
    prompt = f"""
    ЗАДАЧА: Составить ОДНО деловое предложение, где ОБЯЗАТЕЛЬНО ДОСЛОВНО используется фраза "{stopword_word}".
    
    ОБЯЗАТЕЛЬНЫЕ УСЛОВИЯ:
    1. Предложение ДОЛЖНО содержать дословно и в том же регистре фразу "{stopword_word}"
    2. НЕ ИЗМЕНЯЙ фразу "{stopword_word}" - используй её точно в таком виде
    3. Делай предложение в деловом стиле, типичное для рабочей коммуникации
    4. Фраза "{stopword_word}" должна выглядеть уместно в контексте предложения
    5. Не заключай "{stopword_word}" в кавычки, используй как обычную часть речи
