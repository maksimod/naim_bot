...
async def verify_stopword_rephrasing_ai(original_sentence, rephrased_sentence, stopword, user_id=None):
    ...
    if all_stopwords:
        context += f"\nСписок всех стоп-слов, которые должны быть исключены: {', '.join(all_stopwords[:20])}{'...' if len(all_stopwords) > 20 else ''}\n"
        
    # Создаем примеры неправильных замен с синонимами и правильных перефразирований
    examples = f"""
Примеры НЕПРАВИЛЬНЫХ замен (с использованием синонимов):

Стоп-слово: "почему"
Оригинал: "А почему ты это спросил?"
❌ Неправильно: "А зачем ты это спросил?" (заменили на синоним "зачем")
✅ Правильно: "Что побудило тебя задать этот вопрос?" или "С какой целью интересуешься?"

Стоп-слово: "хвастаться"
Оригинал: "Ты достиг многих целей, но не хвастайся этим."
❌ Неправильно: "Ты достиг многих целей, но не гордись этим слишком сильно." (заменили на синоним "гордиться")
✅ Правильно: "Ты достиг многих целей." (Удалили часть с проблемным смыслом, сохранив основную мысль)
✅ Правильно: "Ты достиг многих целей, сохраняй скромность." (Перестроили предложение)

Стоп-слово: "летающая"
Оригинал: "Не поверите, но я видела летающую тарелку!"
❌ Неправильно: "Я видела тарелку" (Изменен основной смысл - обычная тарелка вместо НЛО)
✅ Правильно: "Я наблюдала НЛО!" или "Мне довелось увидеть неопознанный объект в небе!"

Пример со множественными стоп-словами:
Оригинал: "Извините, Вы не так поняли мою предыдущую постановку задачи."
Стоп-слова: "извините", "понять"
❌ Неправильно: "Вы не так поняли мою предыдущую постановку задачи." (убрали только "извините", но оставили "поняли")
✅ Правильно: "Моя предыдущая постановка задачи была истолкована некорректно." (убрали оба стоп-слова)

Текущее стоп-слово: "{stopword_word}"
"""
        
    messages = [
        {
            "role": "system",
            "content": "Вы - строгий эксперт по лингвистике, оценивающий качество перефразирования предложений, содержащих стоп-слова. Проверяйте, что перефразированное предложение НЕ содержит НИКАКИХ стоп-слов из списка и сохраняет ключевой смысл. Обращайте внимание на синонимы стоп-слов, они тоже недопустимы. Оценивайте соответствие смысла оригиналу, обнаруживайте изменения ключевых фактов."
        },
        {
            "role": "user",
            "content": f"Оцените, правильно ли перефразировано предложение:\n\n"
                       ...
                       f"Основное стоп-слово: \"{stopword_word}\"\n\n"
                       f"{context}\n\n"
                       f"КРИТИЧЕСКИ ВАЖНО:\n"
                       f"1. В перефразированном предложении НЕ ДОЛЖНО быть НИКАКИХ стоп-слов из списка, а не только основного стоп-слова.\n" 
                       f"2. Замена любого стоп-слова синонимом считается ОШИБКОЙ (например: почему→зачем, наверное→вероятно).\n"
                       f"3. При сокращении предложения не должен теряться его основной смысл.\n"
                       f"4. Проверяйте ВСЕ слова в перефразированном предложении на соответствие списку стоп-слов.\n\n"
                       f"{examples}\n"
                       f"Верните результат ТОЛЬКО в JSON формате:\n"
                       f"```json\n"
                       f"{{\n"
                       f"  \"preserves_meaning\": true/false - сохраняет ли перефразированное предложение первоначальный смысл,\n"
                       f"  \"excludes_stopword\": true/false - отсутствуют ли ВСЕ стоп-слова И их синонимы в перефразированном предложении,\n"
                       f"  \"used_synonym\": true/false - использовал ли автор синоним вместо какого-либо стоп-слова\n"
                       f"}}\n"
                       f"```"
        }
    ]
